...
//"jsUrl"\s*:\s*("[^"]+")
    ...
    public function getPlayerResponse($page_html)
    {
        if (preg_match('/ytInitialPlayerResponse\s*=\s*({.+?})\s*;/', $page_html, $matches)) {
            ...
            if (empty($ret)) {
                $match = str_replace('"}};ytplayer.web_player_context_config = {"transparentBackground":true,"useFastSizingOnWatchDefault":false,"showMiniplayerButton":true,"externalFullscreen":true,"showMiniplayerUiWhenMinimized":true,"rootElementId":"movie_player', '', $match);
                $ret = json_decode($match, true);
            }
            return $ret;
        }
            ...
            foreach ($formats_combined as $item) {
                ...
                if (isset($item['url'])) {
                    ...
                    $return[] = array(
                        ...
                        'mute' => isset($item['audioQuality']) != '' ? false : true
                    );
